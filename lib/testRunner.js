...
function testRunner() {
	...
	try {
		...
		if (configObj) {
			...
			// Parse CLI arguments to get ignoreCoverageForUntested, if any
			configObj.ignoreCoverageForUntested = cliReader.parseIgnoreCoverageForUntestedCLI(configObj.ignoreCoverageForUntested);
			...
			if (configObj.test && configObj.test.length > 0) {
				server.runTest(0);
			} else if (configObj.src && configObj.src.length > 0 && !configObj.ignoreCoverageForUntested) {
