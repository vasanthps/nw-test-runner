...
var errorCb,
	...
	i,
	j,
	arrayofDuplicateSourceFileNames;
...
function runTest(index, runSourceFiles) {
	...
	if (runSourceFiles) {
		...
		// Name the test result file with full Source file path IF we have identical source file names
		checkDuplicateSourceFileName = configData.src.filter(function (srcPath) {
			/* Hard coding the below path seperator
			   Because the forward slash '/' is used as path seperator in glob node module */
		...
		if (checkDuplicateSourceFileName.length > 1 || arrayofDuplicateSourceFileNames.indexOf(srcFileName) !== -1) {
			outputFilename = utils.getOutputFileName(filePath, outputFilename);
			if (arrayofDuplicateSourceFileNames.indexOf(srcFileName) === -1) {
				arrayofDuplicateSourceFileNames.push(srcFileName);
			}
		}
	...
	if (testData.src.length > 0) {
		try {
			execCmd = '"' + configData.nwPath + '" ./node_modules/nwjs-test-runner/lib/client/ ' + configData.port;
