...
function runTest(index, runSourceFiles) {
    ...
    testData = {};
    testData.outputFolder = configData.output_folder;
    ...
    if (testData.src.length > 0) {
        try {
            execCmd = '"' + configData.nw_path + '" ./node_modules/nw-test-runner/lib/client/ ' + configData.port;
...
successCb = function () {
    ...
    if (i < configData.test.length) {
        ...
        if (j < configData.src.length && !configData.doNotRunCoverage) {
...
function serverCB(socket) {
    ...
    socket.on('data', function (data) {
        ...
        console.log('Client response - ', pdata);
