...
function runTest(index, runSourceFiles) {
	...
	if (runSourceFiles) {
		...
		addlFiles = undefined;
		list = [];
		// Create test results with the folder structure maintained in the source code
		filePath = utils.getRelativeDirPath(configData.testFolder, file);
		fullPath = path.join(configData.output, filePath);
		fsExt.mkdirpSync(fullPath);
		outputFilename = path.join(filePath, outputFilename);
